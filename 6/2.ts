...
const time = parseInt(rawTimes.split(":")[1].split(" ").filter((x) => x.length).join(""))
const record = parseInt(rawDistances.split(":")[1].split(" ").filter((x) => x.length).join(""))
